...
class App extends React.Component {
    ...
    addTodoMethod (newItem) { //newItem is returned by addNewTodo
        this.setState(state => ({
            todoArray: state.todoArray.concat([newItem])
        }))
    }
     //pass 'addTodoMethod' to the component 'AddTodo' as props
     //Whenever setting the new state of component based on the previous state (as we’re doing with our friends todoArray), 
    ...
    render (){
        return (
            <div>
                ...
                <h1>My Todo List</h1>
                <ShowList names={this.state.todoArray} />
            </div>
        )
    }
}
